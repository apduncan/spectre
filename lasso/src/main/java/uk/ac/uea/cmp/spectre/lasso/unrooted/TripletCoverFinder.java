...
/**
 * Locates a maximal chordal subgraph, and then splits this into a number of distinct stable quartet covers
...
public class TripletCoverFinder {
    ...
    /**
     * Convert a potentially incomplete distance matrix into the minimal number of separate quartet covers.
     * @return A set of matrices representing quartet covers
     */
    public List<LassoDistanceGraph> findTripletCovers(ChordalSubgraphFinder.SEED_TREE seedMethod) {
        ...
        for(LassoDistanceGraph component : components) {
            ...
            while (start.isPresent()) {
                ...
                //Continue to get quartet cover sections until no degree 2 vertex exists
    ...
    /**
     * Convert a chordal graph into a minimal number of quartet covers. When triangles joined at only one vertex, can be
     * split into a separate quartet cover. Removes all edges in the located quartet cover from the graph before
     * returning result.
     * @param start A vertex to start searching for a quartet cover from. Should be a vertex of degree 2.
     * @param graph The graph to search for a quartet cover in.
     * @return The quartet cover that contain start vertex.
     */
    ...
    /**
     * Find a valid vertex to start looking for a quartet cover from.
